                ...
                try:
                    ...
                    for _ in range(25):  # Dynamic wait up to 25 seconds for Turnstile
                        ...
                        if match_new:
                            ...
                            # Fire the POST request FROM INSIDE THE BROWSER (via fetch),
                            # not via `requests`. Cloudflare fingerprints the TLS/HTTP
                            # client itself, so a Python-side request gets a 403 even
                            # with valid cookies copied over.
                            fetch_script = """
                            ...
                            """
                            try:
                                result = driver.execute_async_script(fetch_script, post_endpoint)
                                if result and not result.get('error'):
                                    res_headers = {k.lower(): v for k, v in result.get('headers', {}).items()}
                                    if 'hx-redirect' in res_headers:
                                        direct_url = res_headers['hx-redirect']
                                        break
                                    elif 'location' in res_headers:
                                        direct_url = res_headers['location']
                                        break
                            except Exception:
